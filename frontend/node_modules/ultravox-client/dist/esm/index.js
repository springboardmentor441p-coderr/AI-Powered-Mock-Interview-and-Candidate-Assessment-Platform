import { ConnectionState, createLocalAudioTrack, DisconnectReason, Room, RoomEvent, Track, } from 'livekit-client';
...
export var TextMessageUrgency;
(function (TextMessageUrgency) {
...
})(TextMessageUrgency || (TextMessageUrgency = {}));
/* Where a message sent via sendText should be placed if the user is speaking when it is received. */
export var TextMessagePlacement;
(function (TextMessagePlacement) {
...
})(TextMessagePlacement || (TextMessagePlacement = {}));
/* How the agent should proceed after a tool invocation. */
export var AgentReaction;
(function (AgentReaction) {
...
export class UltravoxSession extends EventTarget {
     ...
     */
    constructor({ audioContext, additionalMessages, videoElement, rtcConfig, } = {}) {
        super();
    ...
    sendText(text, optionsOrDeferResponse) {
        if (!UltravoxSession.CONNECTED_STATUSES.has(this._status)) {
    ...
    async handleSocketMessage(event) {
        ...
        this.room.on(RoomEvent.Disconnected, (reason) => this.handleRoomDisconnected(reason));
    ...
    async handleSocketClose(event) {
        ...
        if (!socketClosedNormally) {
            this.dispatchEvent(new UltravoxErrorEvent(new Error(`Session socket closed abnormally. code=${event.code} reason=${event.reason}`)));
        }
    ...
    async handleRoomDisconnected(reason) {
        if (this.isStopped()) {
            return;
        }
