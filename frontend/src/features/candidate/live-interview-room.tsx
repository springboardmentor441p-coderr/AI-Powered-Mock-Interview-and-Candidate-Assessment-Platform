...
import { Disc3, Mic, MicOff, PhoneOff, Radio } from "lucide-react";
...
export default function LiveInterviewRoom() {
  ...
  const beginCall = useCallback(
    async (attempt = 1) => {
      if (!sessionId) return;
      setPhase("connecting");
      // ADDED: reset relay state on every new call attempt
      ...
      try {
        ...
        uvSession.addEventListener("transcripts", async () => {
          ...
          // Refresh current topic whenever a new agent turn finalizes
          ...
          if (hasNewAgentTurn) {
            try {
              ...
              if (res.ok) {
                ...
                console.log("[relay] current question_id:", currentQuestionIdRef.current);
              }
            } catch (e) {
          ...
          // Relay every new final turn to Django
          for (let i = lastSentIdxRef.current + 1; i < all.length; i++) {
            ...
            console.log(`[relay] seq=${i} speaker=${turn.speaker}: ${turn.text.slice(0, 60)}`);
        ...
        console.error(err);
        setPhase("error");
        setErrorMessage(err instanceof Error ? err.message : "Couldn't connect to the interviewer.");
      }
    },
        ...
        {(phase === "preparing" || phase === "connecting") && (
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <p className="font-display text-xl text-foreground">
              {phase === "preparing" ? "Preparing your interview topics…" : "Connecting to the interviewer…"}
            </p>
          ...
          <>
            ...
            <div className="w-full max-w-2xl rounded-xl border border-border bg-card/60 p-5">
              ...
              <div className="flex max-h-72 flex-col gap-3 overflow-y-auto pr-1">
                {lines.length === 0 && (
                  <p className="text-sm text-muted-foreground">The interviewer will begin speaking shortly…</p>
                )}
                {lines.map((line, i) => (
                  <div key={i} className={cn("flex flex-col gap-0.5", line.speaker === "candidate" && "items-end")}>
                    ...
                    <p
                      className={cn(
                        ...
                        line.speaker === "agent" ? "bg-secondary/70 text-foreground" : "bg-primary/15 text-foreground",
